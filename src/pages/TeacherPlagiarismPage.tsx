...
const TeacherPlagiarismPage = () => {
  ...
  const handleViewAssignment = (assignment: Assignment) => {
    ...
    const existingAnalysis = getAnalysisByAssignment(assignment.id);
      ...
      for (let i = 0; i < submissions.length; i++) {
        for (let j = i + 1; j < submissions.length; j++) {
          ...
          const match: PlagiarismMatch = {
            ...
            isFlagged: result.overallScore >= 0.6, // Flag if similarity >= 60%
          };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {assignments.map((assignment) => (
            <AssignmentCard
              ...
              submissionCount={getSubmissionsByAssignment(assignment.id).length}
